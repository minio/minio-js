...
const Package = {
  version: process.env.MINIO_JS_PACKAGE_VERSION || "development",
};
...
export class TypedClient {
  ...
  constructor(params: ClientOptions) {
    ...
    this.reqOptions = {};
    this.pathSuffix = params.pathSuffix ?? "";
  ...
  public setRequestOptions(
    options: Pick<
      ...
      (typeof requestOptionProperties)[number]
    >,
  ) {
    if (!isObject(options)) {
  ...
  private getAccelerateEndPointIfSet(bucketName?: string, objectName?: string) {
    if (
      !isEmpty(this.s3AccelerateEndpoint) && !isEmpty(bucketName) &&
      !isEmpty(objectName)
    ) {
  ...
  setAppInfo(appName: string, appVersion: string) {
    ...
    if (appVersion.trim() === "") {
      throw new errors.InvalidArgumentError(
        "Input appVersion cannot be empty.",
      );
    }
  ...
  protected getRequestOptions(
    ...
    if (virtualHostStyle && !opts.pathStyle) {
      ...
      // path.
      if (bucketName) {
        path = `${this.pathSuffix}/${bucketName}`;
      }
      if (objectName) {
        path = `${this.pathSuffix}/${bucketName}/${objectName}`;
      }
    }
    ...
    if (
      (reqOptions.protocol === "http:" && port !== 80) ||
      (reqOptions.protocol === "https:" && port !== 443)
    ) {
    ...
    return {
      ...reqOptions,
      headers: _.mapValues(
        _.pickBy(reqOptions.headers, isDefined),
        (v) => v.toString(),
      ),
      host,
      port,
  ...
  public async setCredentialsProvider(credentialsProvider: CredentialProvider) {
    if (!(credentialsProvider instanceof CredentialProvider)) {
      throw new Error(
        "Unable to get credentials. Expected instance of CredentialProvider",
      );
    }
  ...
  async makeRequestAsync(
    ...
    return this.makeRequestStreamAsync(
      options,
      payload,
      sha256sum,
      expectedCodes,
  ...
  async makeRequestStreamAsync(
    ...
    if (!this.enableSHA256 && sha256sum.length !== 0) {
      throw new errors.InvalidArgumentError(
        `sha256sum expected to be empty for anonymous or https requests`,
      );
    }
  ...
  protected async getBucketRegionAsync(bucketName: string): Promise<string> {
    ...
    try {
      const res = await this.makeRequestAsync(
        { method, bucketName, query, pathStyle },
        "",
        [200],
        DEFAULT_REGION,
      );
  ...
  async fGetObject(
    ...
    const downloadToTmpFile = async (): Promise<string> => {
      ...
      const downloadStream = await this.getPartialObject(
        bucketName,
        objectName,
        offset,
        0,
  ...
  async statObject(
    ...
    if (!isObject(statOpts)) {
      throw new errors.InvalidArgumentError(
        'statOpts should be of type "object"',
      );
    }
  ...
  async removeObject(
    ...
    if (removeOpts && !isObject(removeOpts)) {
      throw new errors.InvalidArgumentError(
        'removeOpts should be of type "object"',
      );
    }
  ...
  listIncompleteUploads(
    ...
    readStream._read = () => {
      ...
      this.listIncompleteUploadsQuery(
        bucket,
        prefix,
        keyMarker,
        uploadIdMarker,
   ...
   * @internal
   */
  async initiateNewMultipartUpload(
    bucketName: string,
    objectName: string,
    ...
    if (!isObject(headers)) {
      throw new errors.InvalidObjectNameError(
        'contentType should be of type "object"',
      );
    }
    const method = "POST";
    const query = "uploads";
  ...
  async findUploadId(
    ...
    let uploadIdMarker = "";
    for (;;) {
      const result = await this.listIncompleteUploadsQuery(
        bucketName,
        objectName,
        keyMarker,
        uploadIdMarker,
  ...
  protected async listParts(
    ...
    let result;
    do {
      result = await this.listPartsQuery(
        bucketName,
        objectName,
        uploadId,
        marker,
  ...
  async putObject(
    ...
    if (typeof stream === "string" || stream instanceof Buffer) {
      ...
      throw new TypeError(
        'third argument should be of type "stream.Readable" or "Buffer" or "string"',
      );
    }

    if (isNumber(size) && size < 0) {
      throw new errors.InvalidArgumentError(
        `size cannot be negative, given size: ${size}`,
      );
    }
    ...
    if (
      typeof stream === "string" || Buffer.isBuffer(stream) || size <= partSize
    ) {
      const buf = isReadableStream(stream)
        ? await readAsBuffer(stream)
        : Buffer.from(stream);
  ...
  private async uploadStream(
    ...
    let uploadId: string;
    if (!previousUploadId) {
      uploadId = await this.initiateNewMultipartUpload(
        bucketName,
        objectName,
        headers,
      );
      ...
      const oldTags = await this.listParts(
        bucketName,
        objectName,
        previousUploadId,
      );
  ...
  async removeBucketReplication(bucketName: string): Promise<void> {
    ...
    await this.makeRequestAsyncOmit({ method, bucketName, query }, "", [
      200,
      204,
    ], "");
  }
  ...
  async setBucketReplication(
    ...
    if (!isObject(replicationConfig)) {
      throw new errors.InvalidArgumentError(
        'replicationConfig should be of type "object"',
      );
    } else {
      ...
      if (_.isEmpty(replicationConfig.rules)) {
        throw new errors.InvalidArgumentError(
          "Minimum one replication rule must be specified",
        );
      }
    }
    const method = "PUT";
    ...
    await this.makeRequestAsyncOmit(
      { method, bucketName, query, headers },
      payload,
    );
  }
  ...
  async getObjectLegalHold(
    ...
    if (getOpts) {
      if (!isObject(getOpts)) {
        ...
        Object.keys(getOpts).length > 0 && getOpts.versionId &&
        !isString(getOpts.versionId)
      ) {
        throw new TypeError(
          "versionId should be of type string.:",
          getOpts.versionId,
        );
      }
    }
    ...
    const httpRes = await this.makeRequestAsync(
      { method, bucketName, objectName, query },
      "",
      [200],
    );
  ...
  async setObjectLegalHold(
    ...
    if (!isObject(setOpts)) {
      ...
      if (
        ![LEGAL_HOLD_STATUS.ENABLED, LEGAL_HOLD_STATUS.DISABLED].includes(
          setOpts?.status,
        )
      ) {
      ...
      if (setOpts.versionId && !setOpts.versionId.length) {
        throw new TypeError(
          "versionId should be of type string.:" + setOpts.versionId,
        );
      }
    }
  ...
  async getObjectTagging(
    ...
    if (!isObject(getOpts)) {
      throw new errors.InvalidArgumentError(
        'getOpts should be of type "object"',
      );
    }
  ...
  async setBucketPolicy(bucketName: string, policy: string): Promise<void> {
    ...
    if (!isString(policy)) {
      throw new errors.InvalidBucketPolicyError(
        `Invalid bucket policy: ${policy} - must be "string"`,
      );
    }
    ...
    await this.makeRequestAsyncOmit({ method, bucketName, query }, policy, [
      204,
    ], "");
  }
  ...
  async putObjectRetention(
    ...
    if (!isObject(retentionOpts)) {
      throw new errors.InvalidArgumentError(
        'retentionOpts should be of type "object"',
      );
    } else {
      if (
        retentionOpts.governanceBypass &&
        !isBoolean(retentionOpts.governanceBypass)
      ) {
        throw new errors.InvalidArgumentError(
          `Invalid value for governanceBypass: ${retentionOpts.governanceBypass}`,
        );
      }
      if (
        retentionOpts.mode &&
        ![RETENTION_MODES.COMPLIANCE, RETENTION_MODES.GOVERNANCE].includes(
          retentionOpts.mode,
        )
      ) {
        throw new errors.InvalidArgumentError(
          `Invalid object retention mode: ${retentionOpts.mode}`,
        );
      }
      if (
        retentionOpts.retainUntilDate &&
        !isString(retentionOpts.retainUntilDate)
      ) {
        throw new errors.InvalidArgumentError(
          `Invalid value for retainUntilDate: ${retentionOpts.retainUntilDate}`,
        );
      }
      if (retentionOpts.versionId && !isString(retentionOpts.versionId)) {
        throw new errors.InvalidArgumentError(
          `Invalid value for versionId: ${retentionOpts.versionId}`,
        );
      }
    }
  ...
  getObjectLockConfig(
    bucketName: string,
    callback: ResultCallback<ObjectLockInfo>,
  ): void;
  ...
  async setObjectLockConfig(
    ...
    const retentionModes = [
      RETENTION_MODES.COMPLIANCE,
      RETENTION_MODES.GOVERNANCE,
    ];
    const validUnits = [
      RETENTION_VALIDITY_UNITS.DAYS,
      RETENTION_VALIDITY_UNITS.YEARS,
    ];
    ...
    if (lockConfigOpts.mode && !retentionModes.includes(lockConfigOpts.mode)) {
      throw new TypeError(
        `lockConfigOpts.mode should be one of ${retentionModes}`,
      );
    }
    ...
    const isAllKeysSet = ["unit", "mode", "validity"].every((lck) =>
      configKeys.includes(lck)
    );
  ...
  async setBucketVersioning(
    bucketName: string,
    versionConfig: BucketVersioningConfiguration,
  ): Promise<void> {
    if (!isValidBucketName(bucketName)) {
      throw new errors.InvalidBucketNameError(
        "Invalid bucket name: " + bucketName,
    ...
    if (!Object.keys(versionConfig).length) {
      throw new errors.InvalidArgumentError(
        'versionConfig should be of type "object"',
      );
    }
  ...
  private async removeTagging(
    { bucketName, objectName, removeOpts }: RemoveTaggingParams,
  ): Promise<void> {
    const method = "DELETE";
    let query = "tagging";
  ...
  private async applyBucketLifecycle(
    bucketName: string,
    policyConfig: LifeCycleConfigParam,
  ): Promise<void> {
    const method = "PUT";
  ...
  async setBucketLifecycle(
    bucketName: string,
    lifeCycleConfig: LifeCycleConfigParam,
  ): Promise<void> {
    if (!isValidBucketName(bucketName)) {
      throw new errors.InvalidBucketNameError(
        "Invalid bucket name: " + bucketName,
  ...
  async removeObjects(
    ...
    const runDeleteObjects = async (
      batch: RemoveObjectsParam,
    ): Promise<RemoveObjectsResponse[]> => {
      const delObjects: RemoveObjectsRequestEntry[] = batch.map((value) => {
        return isObject(value)
          ? { Key: value.name, VersionId: value.versionId }
          : { Key: value };
      });

      const remObjects = { Delete: { Quiet: true, Object: delObjects } };
      const payload = Buffer.from(
        new xml2js.Builder({ headless: true }).buildObject(remObjects),
      );
  ...
  async removeIncompleteUpload(
    bucketName: string,
    objectName: string,
  ): Promise<void> {
    if (!isValidBucketName(bucketName)) {
      throw new errors.IsValidBucketNameError(
        "Invalid bucket name: " + bucketName,
      );
    }
    if (!isValidObjectName(objectName)) {
  ...
  private async copyObjectV1(
    ...
    if (!isValidBucketName(targetBucketName)) {
      throw new errors.InvalidBucketNameError(
        "Invalid bucket name: " + targetBucketName,
      );
    }
    if (!isValidObjectName(targetObjectName)) {
      throw new errors.InvalidObjectNameError(
        `Invalid object name: ${targetObjectName}`,
      );
    }
    if (!isString(sourceBucketNameAndObjectName)) {
      throw new TypeError(
        'sourceBucketNameAndObjectName should be of type "string"',
      );
    }
  ...
  private async copyObjectV2(
    ...
    if (!(destConfig instanceof CopyDestinationOptions)) {
      throw new errors.InvalidArgumentError(
        "destConfig should of type CopyDestinationOptions ",
      );
    }
    ...
    const headers = Object.assign(
      {},
      sourceConfig.getHeaders(),
      destConfig.getHeaders(),
    );
    ...
    const size = typeof sizeHeaderValue === "number"
      ? sizeHeaderValue
      : undefined;
  ...
  async copyObject(...allArgs: CopyObjectParams): Promise<CopyObjectResult> {
    if (typeof allArgs[0] === "string") {
      ...
      return await this.copyObjectV1(
        targetBucketName,
        targetObjectName,
        sourceBucketNameAndObjectName,
        conditions,
  ...
  async uploadPart(
    ...
    const { bucketName, objectName, uploadID, partNumber, headers } =
      partConfig;
  ...
  async composeObject(
    ...
    | boolean
    | { etag: string; versionId: string | null }
    | Promise<void>
    | CopyObjectResult
  > {
    ...
    if (!Array.isArray(sourceObjList)) {
      throw new errors.InvalidArgumentError(
        "sourceConfig should an array of CopySourceOptions ",
      );
    }
    ...
    if (
      sourceFilesLength < 1 ||
      sourceFilesLength > PART_CONSTRAINTS.MAX_PARTS_COUNT
    ) {
      throw new errors.InvalidArgumentError(
    ...
    const validatedStats = srcObjectInfos.map((resItemStat, index) => {
      ...
      if (
        srcCopySize < PART_CONSTRAINTS.ABS_MIN_PART_SIZE &&
        index < sourceFilesLength - 1
      ) {
        throw new errors.InvalidArgumentError(
      ...
      if (totalSize > PART_CONSTRAINTS.MAX_MULTIPART_PUT_OBJECT_SIZE) {
        throw new errors.InvalidArgumentError(
          `Cannot compose an object of size ${totalSize} (> 5TiB)`,
        );
      }
    ...
    if (
      (totalParts === 1 && totalSize <= PART_CONSTRAINTS.MAX_PART_SIZE) ||
      totalSize === 0
    ) {
      return await this.copyObject(
        sourceObjList[0] as CopySourceOptions,
        destObjConfig,
      ); // use copyObjectV2
    }
    ...
    for (let i = 0; i < sourceFilesLength; i++) {
      (sourceObjList[i] as CopySourceOptions).MatchETag =
        (validatedStats[i] as BucketItemStat).etag;
    }

    const splitPartSizeList = validatedStats.map((resItemStat, idx) => {
      return calculateEvenSplits(
        srcObjectSizes[idx] as number,
        sourceObjList[idx] as CopySourceOptions,
      );
    });
    ...
    const getUploadPartConfigList = (uploadId: string) => {
      ...
      splitPartSizeList.forEach((splitSize, splitIndex: number) => {
        if (splitSize) {
          ...
          const headers = (sourceObjList[splitIndex] as CopySourceOptions)
            .getHeaders();
    ...
    const uploadId = await this.initiateNewMultipartUpload(
      destObjConfig.Bucket,
      destObjConfig.Object,
      newUploadHeaders,
    );
    try {
      ...
      return await this.abortMultipartUpload(
        destObjConfig.Bucket,
        destObjConfig.Object,
        uploadId,
      );
  ...
  async presignedUrl(
    ...
    if (this.anonymous) {
      throw new errors.AnonymousRequestError(
    ...
    if (
      (requestDate && !(requestDate instanceof Date)) ||
      (requestDate && isNaN(requestDate?.getTime()))
    ) {
    ...
    try {
      ...
      if (err instanceof errors.InvalidBucketNameError) {
        throw new errors.InvalidArgumentError(
          `Unable to get bucket region for ${bucketName}.`,
        );
      }
  ...
  async presignedGetObject(
    ...
    validRespHeaders.forEach((header) => {
      // @ts-ignore
      if (
        respHeaders !== undefined && respHeaders[header] !== undefined &&
  ...
  async presignedPostPolicy(postPolicy: PostPolicy): Promise<PostPolicyResult> {
    ...
    try {
      ...
      postPolicy.policy.conditions.push([
        "eq",
        "$x-amz-credential",
        this.accessKey + "/" + getScope(region, date),
      ]);
      postPolicy.formData["x-amz-credential"] = this.accessKey + "/" +
      ...
      const policyBase64 = Buffer.from(JSON.stringify(postPolicy.policy))
        .toString("base64");

      postPolicy.formData.policy = policyBase64;

      postPolicy.formData["x-amz-signature"] = postPresignSignatureV4(
        region,
        date,
        this.secretKey,
        policyBase64,
      ...
      const portStr = this.port == 80 || this.port === 443
        ? ""
        : `:${this.port.toString()}`;
      const urlStr =
        `${reqOptions.protocol}//${reqOptions.host}${portStr}${reqOptions.path}`;
  ...
  async listObjectsQuery(
    bucketName: string,
    prefix?: string,
    marker?: string,
    listQueryOpts?: ListObjectQueryOpts,
  ): Promise<ReturnType<typeof parseListObjects>> {
    if (!isValidBucketName(bucketName)) {
      throw new errors.InvalidBucketNameError(
        "Invalid bucket name: " + bucketName,
      );
    ...
    let { Delimiter, MaxKeys, IncludeVersion } =
      listQueryOpts as ListObjectQueryOpts;
  ...
  listObjects(
    ...
    let ended = false;
    const readStream: stream.Readable = new stream.Readable({
      objectMode: true,
    });
    readStream._read = async () => {
      ...
      try {
        const result: ListObjectQueryRes = await this.listObjectsQuery(
          bucketName,
          prefix,
          marker,
          listQueryOpts,
